...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert sigmoid(a) - (1 - sigmoid(-a)) <= 1e-7
    assert sigmoid(0.5) > 0

    # loop over increasing values
    values = [-2, 0.5, 1, 2, 3]
...
def test_other() -> None:
    ...
    """
    # does relu_back and log_back work?
    assert relu_back(0.5, 0.5) == 0.5
    assert log_back(0.5, 0.5) == 1
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    sumls1 = addLists(ls1, ls2)
    sumls2 = addLists(ls2, ls1)
    assert sumls1 == sumls2
